...
protcseg:
  ...
  call bootmain
    7c45:	e8 c2 00 00 00       	call   7d0c <bootmain>
...
outb(int port, uint8_t data)
{
  ...
  outb(0x1F5, offset >> 16);
    ...
    7ca6:	0f b6 c0             	movzbl %al,%eax
    7ca9:	ee                   	out    %al,(%dx)
  outb(0x1F6, (offset >> 24) | 0xE0);
    7caa:	c1 eb 18             	shr    $0x18,%ebx
...
readseg(uint32_t pa, uint32_t count, uint32_t offset)
{
  ...
  while (pa < end_pa) {
    7cee:	39 fb                	cmp    %edi,%ebx
    7cf0:	73 12                	jae    7d04 <readseg+0x31>
  ...
  while (pa < end_pa) {
    ...
    7cf4:	53                   	push   %ebx
    pa += SECTSIZE;
    7cf5:	81 c3 00 02 00 00    	add    $0x200,%ebx
  while (pa < end_pa) {
    // Since we haven't enabled paging yet and we're using
    // an identity segment mapping (see boot.S), we can
    // use physical addresses directly.  This won't be the
    // case once JOS enables the MMU.
    readsect((uint8_t*) pa, offset);
    7cfb:	e8 7e ff ff ff       	call   7c7e <readsect>
    pa += SECTSIZE;
    offset++;
    7d00:	58                   	pop    %eax
    7d01:	5a                   	pop    %edx
    7d02:	eb ea                	jmp    7cee <readseg+0x1b>
  }
}
    7d04:	8d 65 f4             	lea    -0xc(%ebp),%esp
    7d07:	5b                   	pop    %ebx
    7d08:	5e                   	pop    %esi
    7d09:	5f                   	pop    %edi
    7d0a:	5d                   	pop    %ebp
...
bootmain(void)
{
  ...
  readseg((uint32_t) ELFHDR, SECTSIZE*8, 0);
    7d11:	6a 00                	push   $0x0
    7d13:	68 00 10 00 00       	push   $0x1000
    7d18:	68 00 00 01 00       	push   $0x10000
    7d1d:	e8 b1 ff ff ff       	call   7cd3 <readseg>

  // is this a valid ELF?
  if (ELFHDR->e_magic != ELF_MAGIC)
    7d22:	83 c4 0c             	add    $0xc,%esp
    7d25:	81 3d 00 00 01 00 7f 	cmpl   $0x464c457f,0x10000
    7d2c:	45 4c 46 
    7d2f:	75 38                	jne    7d69 <bootmain+0x5d>
    goto bad;

  // load each program segment (ignores ph flags)
  ph = (struct Proghdr *) ((uint8_t *) ELFHDR + ELFHDR->e_phoff);
    7d31:	a1 1c 00 01 00       	mov    0x1001c,%eax
    7d36:	8d 98 00 00 01 00    	lea    0x10000(%eax),%ebx
  ...
  for (; ph < eph; ph++)
    ...
    7d4b:	73 16                	jae    7d63 <bootmain+0x57>
  ...
  for (; ph < eph; ph++)
    ...
    // as the physical address)
    readseg(ph->p_pa, ph->p_memsz, ph->p_offset);
    7d53:	ff 73 f4             	pushl  -0xc(%ebx)
    7d56:	ff 73 ec             	pushl  -0x14(%ebx)
    7d59:	e8 75 ff ff ff       	call   7cd3 <readseg>
    goto bad;
  ...
  for (; ph < eph; ph++)
    ...
    7d61:	eb e6                	jmp    7d49 <bootmain+0x3d>
    // as the physical address)
    readseg(ph->p_pa, ph->p_memsz, ph->p_offset);
...
outw(int port, uint16_t data)
{
  __asm __volatile("outw %0,%w1" : : "a" (data), "d" (port));
    ...
    7d7c:	eb fe                	jmp    7d7c <bootmain+0x70>
