...
sys_exofork(void)
{
  ...
  // New environment's status is set to ENV_NOT_RUNNABLE.
...
sys_env_set_status(envid_t envid, int status)
{
  ...
  // or the caller doesn't have permission to change envid.
  if ((r = envid2env(envid, &dstenv, 1)) < 0) {
    return r;
  }
...
sys_env_set_trapframe(envid_t envid, struct Trapframe *tf)
{
  ...
  if (!page_lookup(dstenv->env_pgdir, tf, NULL)) {
    panic("sys_env_set_trapframe: page_lookup tf %08x", tf);
  }

  dstenv->env_tf = *tf;
  // tf is modified to make sure that user environments always run at code
  // protection level 3 (CPL 3) with interrupts enabled.
  dstenv->env_tf.tf_cs |= 3; 
  dstenv->env_tf.tf_eflags |= FL_IF;
...
sys_env_set_pgfault_upcall(envid_t envid, void *func)
{
  ...
  dstenv->env_pgfault_upcall = func;
  return 0;
}
...
sys_page_alloc(envid_t envid, void *va, int perm)
{
  ...
  // Return -E_NO_MEM if there's no memory to allocate any
  // necessary page tables.
  if ((r = page_insert(dstenv->env_pgdir, pp, va, perm)) < 0) {
    page_free(pp);
    return r;
  }
...
sys_page_unmap(envid_t envid, void *va)
{
  ...
  page_remove(dstenv->env_pgdir, va);
  return 0;   
}
...
sys_ipc_try_send(envid_t envid, uint32_t value, void *srcva, unsigned perm)
{
  ...
  // Return -E_IPC_NOT_RECV if envid is not currently blocked
  // in sys_ipc_recv, or another environment managed to send first.
  ...
  // Return -E_INVAL if srcva < UTOP but srcva is not mapped in the caller's
  // address space.
  if ((uintptr_t)srcva < UTOP &&
  ...
  // Return -E_NO_MEM if there's not enough memory to map srcva in envid's
  // address space.
  r = -1;
  if ((uintptr_t)srcva < UTOP &&
  ...
  dstenv->env_ipc_perm = (r != -1 ? perm : 0);
 
  // Set target environment's sys_ipc_recv return value to 0.
...
sys_ipc_recv(void *dstva)
{
  ...
  curenv->env_ipc_dstva =
    dstva < (void*)UTOP ? dstva : (void*)UTOP;
...
// -E_INVAL if len is greater than maximal Ethernet packet size. 
int
sys_try_transmit_pkt(void *srcva, size_t len)
{
  if (len > PKTSIZE) {
    return -E_INVAL;
  }

  user_mem_assert(curenv, srcva, len, 0);

  return try_transmit_pkt(srcva, len);
} 

// Try to receive a packet from E1000.
//
// Returns length of packet on success, < 0 on error.
// Errors are:
// -E_NO_MEM if the receive array of E1000 is empty.
int
sys_try_receive_pkt(void *dstva)
{
  // Check environment have permission to write
  // at least a single maximal Ethernet packet. 
  user_mem_assert(curenv, dstva, PKTSIZE, PTE_W);
 
  return try_receive_pkt(dstva);
}
...
syscall(uint32_t syscallno, uint32_t a1, uint32_t a2, uint32_t a3, uint32_t a4, uint32_t a5)
{
  ...
  case SYS_try_transmit_pkt:
    return sys_try_transmit_pkt((void*)a1, a2);
  case SYS_try_receive_pkt:
    return sys_try_receive_pkt((void*)a1);
  default:
    return -E_INVAL;
  }
}
